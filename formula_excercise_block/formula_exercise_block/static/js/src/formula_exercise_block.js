...
function FormulaExerciseXBlock(runtime, xblockElement) {
	...
	function handleSubmissionResult(results) {
		...
		// Remove previous per-expression feedback
		student_view_expressions_table_element.find('span.fe-feedback').remove();
		var exprResults = results['expression_results'] || {};
		student_view_expressions_table_element.find('tr').each(function(){
			var columns = $(this).find('td');
			var name = columns.eq(1).children().eq(0).text();
			if(!name) return;
			if(exprResults.hasOwnProperty(name)){
				var status = exprResults[name];
				var span = $('<span></span>').addClass('fe-feedback');
				if(status === 1){ span.addClass('fe-correct').text(' Correct!'); }
				else { span.addClass('fe-incorrect').text(' Incorrect.'); }
				columns.eq(3).append(span);
			}
		});
		var overall = results['overall_feedback'];
		if (overall){
			var overallDiv = $(xblockElement).find('#fe-overall-feedback');
			if(!overallDiv.length){
				overallDiv = $('<div id="fe-overall-feedback" class="fe-overall-feedback"></div>');
				student_view_expressions_table_element.after(overallDiv);
			}
			overallDiv.text(overall);
			overallDiv.removeClass('correct incorrect');
			if(overall.toLowerCase().startsWith('correct')) overallDiv.addClass('correct'); else overallDiv.addClass('incorrect');
		}
  	}

    // helper not strictly needed now, but kept for future use
    function pointsEarnedFrom(results){
        return results['overall_feedback'] && results['overall_feedback'].toLowerCase().startsWith('correct');
    }
	...
	$(xblockElement).find('input[name=submit-button]').bind('click', function() {
		...
		student_view_expressions_table_element.find('tr').each(function(row_index) {
			var columns = $(this).find('td');
			var name = columns.eq(1).children().eq(0).text(); // expression name
			var value = columns.eq(3).children().eq(0).val(); // expression value
			submitted_expression_values[name] = value;
			console.log('[FormulaExercise] Row ' + row_index + ' expression ' + name + ' = ' + value);
		});
		...
		console.log('[FormulaExercise] Submitting payload', data);
		...
		$.ajax({
			...
			dataType: 'json'
		}).done(handleSubmissionResult).fail(function(jqXHR){
			console.error('[FormulaExercise] submit failed', jqXHR.status, jqXHR.responseText);
		});
	});
    ...
    $(function($) {
		...
		if (show_answer_button != null) {
			show_answer_button.bind('click', function() {
				console.log('[FormulaExercise] show_answer_button CLICKED');
				var data = {
					saved_question_template: hidden_question_template_element.val(),
		  			serialized_variables: hidden_variables_element.val(),
		  			serialized_expressions: hidden_expressions_element.val(),
				...
				$.ajax({
					...
					contentType: 'application/json',
					dataType: 'json'
				}).done(handleShowAnswerResult).fail(function(jqXHR){
					console.error('[FormulaExercise] show_answer failed', jqXHR.status, jqXHR.responseText);
				});
			});
		}
    });
