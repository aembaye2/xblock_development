...
class FormulaExerciseXBlock(StudioEditableXBlockMixin, ScorableXBlockMixin, XBlock):
    ...
    frag = Fragment()
    # Use render_django_template (render_template deprecated in newer XBlock releases)
    ...
    def student_submit(self, data, suffix=''):
        ...
        # Store scorable values
        self._raw_earned = float(points_earned)
        self._raw_possible = float(self.max_points)
        
        submit_result = {}
        submit_result['point_string'] = self.point_string
        # Include per-expression correctness mapping for frontend feedback
        submit_result['expression_results'] = evaluation_result
        # Overall feedback string (any incorrect makes overall incorrect)
        if all(val == 1 for val in evaluation_result.values()):
            submit_result['overall_feedback'] = 'Correct!'
        else:
            submit_result['overall_feedback'] = 'Incorrect.'
    ...
    def has_submitted_answer(self):  # pragma: no cover simple predicate
        return self.attempt_number > 0

    def get_score(self):  # pragma: no cover trivial accessor
    ...
    def calculate_score(self):  # pragma: no cover
        return Score(raw_earned=self._raw_earned, raw_possible=self._raw_possible)

    @property
    def student_item_key(self):  # replacement for legacy mixin
