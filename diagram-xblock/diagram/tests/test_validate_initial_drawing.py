...
from diagram.diagram import validate_initial_diagram, LINE


def test_validate_accepts_full_canvas_object():
    canvas = {"version": "5.5.2", "objects": LINE.get('objects', [])}
    out = validate_initial_diagram(canvas)
...
def test_validate_accepts_list_of_objects():
    ...
    out = validate_initial_diagram(obj_list)
    assert isinstance(out, list)


def test_validate_rejects_invalid_type():
    with pytest.raises(ValueError):
        validate_initial_diagram('this-is-not-valid')


def test_validate_truncates_large_number_of_items():
    big = [LINE['objects'][0]] * 1000
    out = validate_initial_diagram(big, max_items=10)
    assert len(out) == 10
import json
import pytest
import ast
...
# Import the validate_initial_diagram function from the module source without
...
validate_initial_diagram = locs.get("validate_initial_diagram") or globs.get("validate_initial_diagram")
