...
try:
    ...
    # Minimal fallback for Fragment when web_fragments isn't available (workbench/dev environments)
    class Fragment:
        ...
        def initialize_js(self, name, data=None):
            # No-op in fallback; store for debugging
            self._init = (name, data)
            return self
...
except Exception:  # submissions service is optional (workbench may not provide it)
...
try:
    ...
    # Workbench or lightweight dev env won't have the full Open edX LMS.
    # Provide a minimal fallback so imports don't fail; callers should
    # be tolerant of the dummy response (no course settings).
