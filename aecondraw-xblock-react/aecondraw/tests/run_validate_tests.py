...
def load_validator():
    src_path = pathlib.Path(__file__).resolve().parents[1] / "aecondraw.py"
    ...
    if func_node is None:
        raise RuntimeError("validate_initial_drawing not found in aecondraw.py")
    ...
    return locs.get("validate_initial_drawing") or globs.get("validate_initial_drawing")
...
def run_tests():
    validate_initial_drawing = load_validator()
