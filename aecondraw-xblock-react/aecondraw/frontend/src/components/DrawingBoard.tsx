...
interface DrawingBoardProps {
  ...
  // Bounding box for board [xMin, yMax, xMax, yMin]
...
export default function DrawingBoard({ 
  ...
  const boundingBox: [number, number, number, number] = customBoundingBox || [-1, 11, 11, -1];
  ...
  const getCurrentState = (): BoardState => {
    ...
    for (const id in board.objects) {
      ...
      if (obj.elType === 'axis' || obj.elType === 'ticks' || obj.elType === 'grid' || 
          obj.name === axisLabels[0] || obj.name === axisLabels[1]) {
        continue;
      }
