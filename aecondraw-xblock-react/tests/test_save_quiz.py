...
"""Tests for AecondrawXBlock save_quiz handler"""
...
from aecondraw import AecondrawXBlock


class TestDrawingSaveQuiz(TestCase):
    def test_save_quiz_updates_fields(self):
        ...
        block = AecondrawXBlock(ToyRuntime(), scope_ids=scope_ids)
        ...
        payload = {
            ...
            'hideLabels': True,
            'initialDrawing': [{'type': 'rect', 'left': 10, 'top': 20}],
        }
        ...
        self.assertEqual(list(block.initial_drawing), list(payload['initialDrawing']))
