...
class DrawingBlock(
    ...
    @XBlock.json_handler
    def submit_line(self, data, suffix=''):
        """
        Accepts two points from the frontend, checks if the line has a positive slope.
        Expects data = {"start": {"x": float, "y": float}, "end": {"x": float, "y": float}}
        Returns: {"correct": bool, "slope": float, "feedback": str}
        """
        try:
            start = data["start"]
            end = data["end"]
            ...
            return {"correct": False, "feedback": f"Invalid input: {e}"}

        if x2 == x1:
            return {"correct": False, "slope": None, "feedback": "Vertical line: slope is undefined. Please draw a line with positive slope."}

        slope = (y2 - y1) / (x2 - x1)
        # In canvas, y increases downward, so negative slope means positive slope visually
        is_positive = slope < 0
        feedback = "Correct!" if is_positive else "Incorrect. Please draw a line with positive slope."
        return {"correct": is_positive, "slope": slope, "feedback": feedback}
    ...
    def workbench_scenarios():
        ...
        return [
            # (
            #     "Drawing standard",
            #     "<vertical_demo><drawing/></vertical_demo>"
            # ),
            (
                "Drawing assessment",
                "<vertical_demo><drawing mode='assessment' max_attempts='3'/></vertical_demo>"
            ),
        ]
